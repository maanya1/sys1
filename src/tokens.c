...
// Appends a node to a token linked list.
...
// Merges two token linked lists distinctly.
...
// Appends a node to a token linked list distinctly.
...
// Gets the length of a token linked list.
int Token_length(Token* list) {
  if (list == NULL) return 0;
  return 1 + Token_length(list->next);
}

// Converts a token linked list to a string.
...
// Reads a file into a list distinctly.
